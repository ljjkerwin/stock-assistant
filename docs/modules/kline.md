...
- **数据源选型**：东方财富 push2his/push2 虽对高频抓取 K线（大量历史）存在限制，但对于单次的分时线获取十分友好，腾讯财经 ifzq（原生支持前复权、抓取宽松、国内直连）适合做 K线主源。`fetchBars` 按周期/市场路由：
  - **分时线**（`timeshare`，即当天1分钟线，不复权）：A股/ETF 走腾讯财经 `mkline` `m1` 接口，无延迟、访问快且不需要并发限制队列；港股优先尝试东方财富实时分时接口 `http://push2.eastmoney.com/api/qt/stock/trends2/get`（通过 `RequestQueue` 限制最大并发为 1 并发调度），但在发生任何网络超时或接口异常时，**自动且无感知降级到 Yahoo Finance 分时接口进行兜底**，确保数据展示的绝对稳定性。
  ...
  - **分钟线**（`1min`/`5min`/`15min`/`30min`/`60min`，不复权）：A股/ETF 走腾讯 `ifzq.gtimg.cn/appstock/app/kline/mkline`（周期码 `m1/m5/m15/m30/m60`，分时使用 `m1`）；港股腾讯不提供分钟线，仍走 **Yahoo Finance** `query1.finance.yahoo.com/v8/finance/chart`
