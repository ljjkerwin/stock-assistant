...
- 路由：`/darktrade-discovery`，位于 Sidebar「总览」下方；调用 `GET /api/darktrade/discovery` 展示暗盘资金大于 2,000 万元且严格超过明盘资金两倍的标的。
- 页面进入只读取现有索引；「刷新全市场数据」由用户手动确认后调用 `POST /api/darktrade/refresh-index` 重建全市场索引，完成后自动重新筛选列表，避免普通浏览触发高开销抓取。
- 鼠标悬停股票标题会打开固定定位浮窗：左侧展示当日分时主图、成交量与分钟粒度明暗盘副图，右侧展示日线主图、日线成交量与日粒度明暗盘副图；点击标题仍进入股票详情页。
