...
- 基金详情页的前 10 大持仓个股 hover 时复用 `HoldingKlinePopup` 展示近 9 个月日 K；浮窗宽 920px、图表高 440px。图中最多标注最近三期持仓公告对应的截止时间，标记文字直接使用 `YYYY-MM-DD` 日期，不显示「持仓日」。该股在当期持仓中时标红，未持有的历史期标灰；非交易日自动落在此前最近一个交易日 K 线上。
