...
| GET    | `/api/auth/plugin-tokens`     | 获取当前用户的插件访问令牌列表（不返回令牌明文）                                                                                                       |
| POST   | `/api/auth/plugin-tokens`     | 创建插件访问令牌。body `{ name, expiresInDays? }`，有效期 1–365 天、默认 180 天；仅本次返回 `{ id, name, token, expiresAt }`，`token` 以 `xhs_` 开头。 |
| DELETE | `/api/auth/plugin-tokens/:id` | 撤销当前用户的一枚插件访问令牌。                                                                                                                       |
...
> 插件访问令牌独立于登录 JWT，数据库仅保存 SHA-256 哈希。`xhs_` 令牌只能请求 `POST /api/darktrade/daily-result-from-text`，每枚令牌每分钟最多 60 次；其他 API 一律按普通登录 JWT 鉴权。
...
| GET  | `/api/darktrade/discovery?minDarkCapital=&minMultiple=&capitalDirection=&date=` | 明暗盘挖掘列表：直接查询指定日期（`date` 为 `YYYYMMDD`，省略时默认当天）的 `dark_trade_daily_result` 每日结果表；每只股票仅一条最新快照记录。`capitalDirection` 可选 `inflow`（默认，暗盘资金流入严格超过阈值）或 `outflow`（暗盘资金流出绝对值严格超过阈值）；两种模式均要求暗盘资金绝对值严格大于明盘资金绝对值的 `minMultiple` 倍。流入按资金金额降序、流出按流出金额绝对值降序返回 `DarkTradeData[]`。两个资金参数均为非负数，可省略，默认分别为 `30,000,000` 和 `2`。历史日期的结果表为空时，接口会从既有快照一次性回填。                                                                                                                                                  |
| GET  | `/api/darktrade/daily-result?name=&date=`                     | 按股票名称和日期查询一条已归档的**15:00 收盘**明暗盘资金，`name` 必填，支持完整/部分中文、拼音首字母及混合输入（如 `华电ln`、`zjxc`）；多候选时依次优先完整名称、中文名称前缀（如 `康德` 优先“康德莱”而非“药明康德”）、完整拼音首字母（如 `dfd` 优先“多氟多”而非“东方电子”）和其他部分匹配。`date` 为可选 `YYYYMMDD`（默认当天）。返回 `DarkTradeData`（额外含 `displayName`：随机保留前 1 或 2 个汉字，后续汉字转为拼音首字母）、随机 `summarySuffix` 与后端统一生成的最终文案 `summary`，或 `null`。若命中股票当天仅有盘中资金、没有 15:00 快照，返回 400 错误；若该日每日结果表尚未建立，会从已有快照一次性回填。 |
| POST | `/api/darktrade/daily-result-from-text`                       | 收盘暗盘资金 LLM 查询。body 为 `{ text, date? }`；后端通过硅基流动的 `Qwen/Qwen3.5-4B` 从一句话提取最多 12 个股票/场内 ETF 名称，再复用日终查询返回 `{ names, results, notFoundNames, summarySuffix, summary }`。`summary` 是后端统一生成的最终可复制文案，供管理中心与小红书插件直接使用；`summarySuffix` 从空文本、` ～`、` ～～`、`[吧唧R][吧唧R]`、`[吃瓜R]`、`[黄金薯R][黄金薯R]`、`[暗中观察R]` 中随机选取。在后端配置 `SILICONFLOW_API_KEY`，密钥不下发前端。                                                                                                                                                                                                                                                                                                               |
...
| GET  | `/api/darktrade/snapshots/:code?days=`                        | 单只股票的历史暗盘快照（**日粒度**：历史交易日取 15:00 收盘快照；当天未收盘时以最新盘中快照作为当日临时值，默认 60 天），返回 `DarkTradeSnapshot[]`，`time` 为 `YYYY-MM-DD`                                                                                                                                                                                                                                                                                                                                                                               |
...
`daily-result` 的 `name` 参数也支持六码股票代码（如 `002922`）；`daily-result-from-text` 会从原文直接提取独立出现的六码代码并精确查询，不依赖模型是否识别数字。
