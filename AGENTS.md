...
- 本地推送检查：配置了 Git pre-push 钩子（`.git/hooks/pre-push`），在本地执行 `git push` 时，若检测到本次推送包含 `frontend` 目录下的变更，会运行完整的前端 TypeScript 类型检查；若包含 `backend` 目录下的变更，会运行后端单元测试，避免将错误代码推送。
