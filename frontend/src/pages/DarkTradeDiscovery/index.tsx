...
export default function DarkTradeDiscovery() {
  ...
  const [hovered, setHovered] = useState<{ code: string; name: string; rect: DOMRect } | null>(null);
  const leaveTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      setData(await darktradeApi.getDiscoveryStocks());
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const columns: ColumnsType<DarkTradeData> = [
    {
      ...
      render: (_, record) => (
        ...
        >
          ...
          <Text type="secondary" className={styles.code}>{record.code}</Text>
        </Button>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (value: number | null) => (
        <span className={value != null && value < 0 ? styles.down : styles.up}>{formatPercent(value)}</span>
      ),
    },
    {
      title: '暗盘资金',
      ...
      align: 'right',
      render: (value: number | null) => <strong className={styles.darkCapital}>{formatCapital(value)}</strong>,
    },
    {
      title: '明盘资金',
    ...
    },
    {
      ...
      align: 'right',
      render: (_, record) => {
        if (record.darkCapital == null || record.lightCapital == null || record.lightCapital === 0) return '--';
        return `${(record.darkCapital / record.lightCapital).toFixed(2)} 倍`;
      },
    },
    {
      title: '行业',
  ...
  return (
    <div className={styles.page}>
      <div className={styles.header}>
        <div>
          <Title level={4} className={styles.title}>明暗盘挖掘</Title>
          <Text type="secondary">筛选暗盘资金超过 2,000 万，且大于明盘资金 2 倍的股票</Text>
        </div>
        <Space>
          <Popconfirm
            title="刷新全市场明暗盘数据？"
          ...
          >
            <Button icon={<CloudSyncOutlined />} loading={refreshingIndex} disabled={loading && !refreshingIndex}>
              刷新今天全市场数据
            </Button>
          </Popconfirm>
        </Space>
      ...
      <Table<DarkTradeData>
        ...
        dataSource={data}
        loading={loading}
        pagination={{ pageSize: 50, showSizeChanger: false, showTotal: (total) => `共 ${total} 只` }}
